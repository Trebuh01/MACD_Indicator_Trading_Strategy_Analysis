...
def load_and_calculate_indicators(file_path):
    ...
    # Obliczanie EMA26, EMA12, MACD i linii sygnałowej
...
def plot_with_signals2(data):
    ...
    plt.scatter(data.index[cross_down], data['MACD'][cross_down], color='red', marker='v', label='Sygnał Sprzedaży', alpha=1)
...
def simulate_trading(data, positions_limit):
    ...
    # Sprzedaj pozostałe pozycje na koniec okresu
...
def plot_with_signals(data, transactions):
    ...
    plt.scatter(transactions['buy'], data.loc[transactions['buy']]['Close'], label='Zakup', marker='^', color='green', alpha=1)
    plt.scatter(transactions['sell'], data.loc[transactions['sell']]['Close'], label='Sprzedaż', marker='v', color='red', alpha=1)
